...
class WebUI:
    ...
    async def _http_request_get_websocket(self, request):
        ...
        self.logger.info("return websocket")
    ...
    async def _midi_bridge_send_midi_inputs(self):
        midi_inputs = self._midi_bridge.get_inputs()
        await self._send_websocket_message({
            'response': 'clear_midi_inputs',
            'params': {
                'count': 0
            }
        })

        for index, input in enumerate(midi_inputs):
            await self._send_websocket_message({
                'response': 'midi_input',
                'params': {
                    'id': index + 1,
